...
namespace IuvoUnity
{
    namespace StateMachines
    {
        namespace CSM
        {
            ...
            {
                [Expandable]
                public ConditionalState currentState;
                [Expandable]
                public ConditionalState defaultState;
                [Expandable]
                public ConditionalState previousState;

                [Header("States")]
                public List<ConditionalState> States = new List<ConditionalState>();

                [Tooltip("Queue that tracks the last (historySize) states entered")]
                public Queue<ConditionalState> History = new Queue<ConditionalState>(); // is only runtime
                [SerializeField] private int historySize = 10; // default history size

                // called whenever a state is entered
                private void AddToHistory(ConditionalState state)
                {
                    if (state != null)
                    {
                ...
                public bool IsInState(ConditionalState state) => currentState == state;
                public bool WasInState(ConditionalState state) => previousState == state;
                ...
                private void HandleContinue(ConditionalStateMachineUpdateMode mode)
                {
                ...
                public virtual void Update() => HandleContinue(ConditionalStateMachineUpdateMode.Update);

                public virtual void FixedUpdate() => HandleContinue(ConditionalStateMachineUpdateMode.FixedUpdate);

                public virtual void LateUpdate() => HandleContinue(ConditionalStateMachineUpdateMode.LateUpdate);

                public virtual bool TryChangeState(ConditionalState newState)
                {
                    if (newState != null)
                    {
                ...
                public virtual bool ChangeState(ConditionalState newState, bool canEnterNew)
                {
