        ...
        namespace HSM
        {
            public class StateMachineBuilder
            {
                ...
                void Wire(State s, StateMachine m, HashSet<State> visited)
                {
                    ...
                    {
                        if (!typeof(State).IsAssignableFrom(fld.FieldType)) continue; // Not a State field
                        if (fld.Name == "Parent") continue; // Skip Parent field

                        var child = (State)(fld.GetValue(s));
                        if (child == null) continue; // Null field
                        if (!ReferenceEquals(child.Parent, s)) continue; // Not a child of 's'
